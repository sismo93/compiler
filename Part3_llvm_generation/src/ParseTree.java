...
public class ParseTree {
     ...
     * @param chdn Its children
     */
    public ParseTree(String lbl, List<ParseTree> chdn){
    ...
    /**
     * Creates a tree with root labeled by lbl and children chdn
     * remove all EPS from the tree
     * it will help us when will have to go through all the children
     * @param  lbl  The label of the root
     * @param chdn Its children
     * @param ruleNumber the rule given by our grammar
    */
    public ParseTree(String lbl,List<ParseTree> chdn,int ruleNumber) {
        this.rule = ruleNumber;
        this.label = new Symbol(lbl);
        List<ParseTree> listNoEPS = new ArrayList<ParseTree>(); 

        for  (ParseTree pars : chdn){
        	if (pars!=null){
        		listNoEPS.add(pars);
        	}
        }
        this.children = listNoEPS;
    }
    ...
    */    
    ...
    /**
     * Check each token and write it in the right latex syntax
     * @return the right syntax in latex for each token
     */
    public String toTeX(){ // transform into the right codage for latex
        if (label.isEpsilon()){
            return "$\\varepsilon$";
        } else if (label.isEOS()){
            return "$\\dashv$";
        }else if (label.isParen() || label.isCompEqual()){
            return "$"+label.getType()+"$"+" \\texttt{"+label.getValue()+"}";
        } else {
            final String value = label.getValue() != null? label.getValue().toString() : "";
            final String type  = label.getType()  != null? label.getType().toString()  : "";
